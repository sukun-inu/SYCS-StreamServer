...
│   OBS入力   →  /hls/live/{key}/index.m3u8 (high)    │
│                                                      │
│  FFmpeg (publish.sh)                                 │
│   RTSP → 720p VBR → RTMP → mediamtx再受信           │
│             →  /hls/live/{key}_transcode/ (low)      │
└────────────────────────┬─────────────────────────────┘
...
│    /hls/live/{key}/index.m3u8  (LL-HLS ブロッキング) │
│                                                      │
...
└── api-server/
    ├── Dockerfile
    ├── requirements.txt
    └── main.py               FastAPI: WS プッシュ + 署名配信 + ポータル
```
...
docker compose logs media-server | grep -E "GPU|nvenc|encoder"
```
