...
FastAPI (api-server :8080)
 ...
 │   GET  /seg/{key}/seg*.mp4?exp=T&sig=S
...
`active` は MediaMTX HLS (`{key}/stream.m3u8` または `{key}/index.m3u8`) の取得可否で判定。
