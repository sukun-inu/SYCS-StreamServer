...
│   OBS入力   →  MediaMTX HLS /{key}/index.m3u8       │
│                                                      │
│  FFmpeg (publish.sh)                                 │
...
│                                                      │
│  ブラウザ向け: hls.js の HTTP LL-HLS で再生          │
...
│  127.0.0.1:1935 → rtmp://x.tcp.ngrok.io:PORT/{key}  │
└──────────────────────────────────────────────────────┘
```
...
|------|-----|
| 設定 → 配信 → サービス | **カスタム...** |
| サーバー | ポータルの「RTMP URL」欄の URL (例: `rtmp://0.tcp.jp.ngrok.io:XXXXX/kawasaki`) |
| ストリームキー | **空欄** |

> 視聴 URL 生成欄に入力したキーが RTMP URL の末尾に入ります。
> MediaMTX は OBS の「サーバーURLのパス」をストリーム名として扱うため、OBS のストリームキー欄は空にしてください。
...
1. **視聴 URL 生成** — ストリームキー欄に配信名を入力する。
2. **OBS 配信設定** — RTMP URL が配信名入りで表示される。OBS のストリームキー欄は空にする。
...
- hls.js のネイティブ HTTP LL-HLS で再生
- MediaMTX の `session` クエリを保持して init / part / segment をプロキシ
- WebSocket プレイリスト経路は互換用としてコード上に残すが、標準では使用しない
...
表示された `rtmp://x.tcp.ngrok.io:PORT/{ストリームキー}` を OBS のサーバーに設定し、OBS のストリームキー欄は空にしてください。
...
よくある原因:
- `/api/stream/{key}` が `active:false` を返している → MediaMTX の `/{key}/index.m3u8` が読めていない
- `*_init.mp4` が 404 になる → MediaMTX の `session` クエリがプロキシで落ちている

### 遅延が大きい / 遅延が増え続ける

1. OBS のキーフレーム間隔を **1 秒固定** に設定する
2. ブラウザの DevTools → Network で `/hls/live/{key}/index.m3u8` と `*_init.mp4` が 200 を返しているか確認
...
ブラウザ視聴は標準で HTTP LL-HLS を使うため、Cloudflare 側では通常のHTTPと長めのレスポンスタイムアウトを通してください。
