...
| プレイリスト配信 | 標準は hls.js の **HTTP LL-HLS**、WebSocket プッシュは互換経路 |
| セグメント配信 | HTTP LL-HLS は `/hls` プロキシ、WebSocket互換経路は HMAC 署名付き `/seg/` |
...
| 公開ポート | `1935/tcp` (RTMP) |
| 書き込みパス | high: `/hls/{key}/`、low: `/hls/live/{key}_transcode/` |

**ストリーム構成:**

```
OBS (publisher)
    │ rtmp://host:1935/{key}  (OBS の Stream Key は空)
    ▼
mediamtx
    │ ── LL-HLS ネイティブ生成 ──────────────────────────────────────────
    │     /hls/{key}/...                 (OBS 元解像度、high)
    │
    └── runOnReady → publish.sh  (flock 排他制御)
         │ MTX_PATH="{key}" → STREAM_NAME="{key}"
         │ "_transcode" サフィックスのパスは即 exit 0 (再帰防止)
         └── FFmpeg  (RTSP → 720p VBR → RTMP)
              出力: rtmp://127.0.0.1:1935/live/{key}_transcode
                    ↓ mediamtx が再受信
                    /hls/live/{key}_transcode/...  (720p、low)
```
...
| 役割 | MediaMTX HLS プロキシ、WebSocket互換プレイリスト配信、ポータルサイト、セッション管理 |
| ホストポート | `8080/tcp` |
| 読み取りパス | `/hls/` (read-only マウント) |
| 主要依存 | fastapi, uvicorn[standard], watchfiles, aiofiles, httpx |

**HTTP LL-HLS 配信 (ブラウザ標準経路):**

```
GET /hls/live/{key}/master.m3u8         ← 動的生成
GET /hls/live/{key}/index.m3u8          ← MediaMTX の media playlist へ解決して返す
GET /hls/live/{key}/*.mp4?session=...   ← MediaMTX session を保持してプロキシ
```

**WebSocket LL-HLS 配信 (互換経路):**
...
```
OBS
 │ RTMP :1935/{key}  (OBS Stream Key は空)
 ▼
mediamtx (media-server)
 │ LL-HLS ネイティブ生成 ────────────────────────────────────────────────
 │   http://127.0.0.1:8888/{key}/index.m3u8
 │   /hls/{key}/seg*.mp4             ← hlsDirectory に保存されるセグメント
 │
 └── runOnReady → publish.sh
      └── FFmpeg (RTSP:8554 → 720p → RTMP:1935/live/{key}_transcode)
           ↓ mediamtx が再受信
...
├─ [ブラウザ視聴ページ /watch/{key}]
 ...
 │        → セッション確保、利用可能variantだけでmaster生成
 │   GET  /hls/live/{key}/index.m3u8?_HLS_msn=N&_HLS_part=P&sid=S
 │        → MediaMTX HLS から取得 → パッチ済みで返す
 │   GET  /hls/live/{key}/*.mp4?session=M&sid=S
 │        → hlsDirectory または MediaMTX から即時プロキシ
 │
 ├─ [VRChat / 直 URL アクセス]
 │   GET  /hls/live/{key}/master.m3u8
...
*hls.js のネイティブ LL-HLS ブロッキングリロードを使い、MediaMTX 由来の `session` クエリを保持して余分な中継を避ける。*
...
| hls.js HTTP LL-HLS | WebSocket中継を避け、MediaMTXのブロッキングリロードをそのまま使う |
...
LL-HLS の playlist (`index.m3u8` から辿れる media playlist) は api-server が `MEDIAMTX_HLS_URLS` の候補から取得する。`/hls` volume はセグメント保管と診断用であり、LL-HLS では `.m3u8` が保存されない場合がある。
...
| `PART-HOLD-BACK` 安定化 | `_patch_playlist` (全経路) | `HLS_PART_HOLD_BACK_PARTS × PART-TARGET` を下限にして、Cloudflare越しのライブエッジ追従を安定化 |
| low-first master | 動的 master.m3u8 | 単体m3u8再生や初期選択で720p lowから入る |
| low固定 | hls.js 設定 | ブラウザ視聴ページは標準で最低ビットレートlevelに固定 |
...
| 脅威 | 対策 |
|------|------|
| DevTools から m3u8 URL をコピーして再生 | `sid` によるセッション管理と帯域制限で制御 |
| MediaMTX session の改ざん | WebSocket互換経路の `/seg` は `src` を署名対象に含める |
| WebSocket token の使い回し | 互換経路ではワンタイム消費 (取得後 60 秒 TTL、接続で即失効) |
...
`active` は MediaMTX HLS の公式パス (`{key}/index.m3u8`) の取得可否で判定する。旧互換の `stream.m3u8` はフォールバック扱い。
...
```json
{"site": "https://...", "rtmp_base": "rtmp://...", "rtmp_local_base": "rtmp://192.168.x.x:1935", "sessions": {"active": 3, "max": 100}}
{"ping": true}
```
...
視聴者向け hls.js プレイヤーページ。標準では HTTP LL-HLS でプレイリストとセグメントを取得する。
...
| `MEDIAMTX_API_URLS` | `http://127.0.0.1:9997,http://media-server:9997,http://sycs-media-server:9997` | 診断APIがMediaMTXの実パス一覧を読む内部URL候補 |
...
| `HLS_PART_HOLD_BACK_PARTS` | `6` | `PART-HOLD-BACK` の最低パーツ数。小さいほど低遅延、大きいほど安定 |
| `HLS_MEDIA_CACHE_TTL` | `30` | MediaMTX から取得した init / part / segment のメモリキャッシュ秒数 |
| `HLS_MEDIA_CACHE_MAX_BYTES` | `67108864` | mediaメモリキャッシュ上限バイト数 |
