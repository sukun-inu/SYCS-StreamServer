...
**high バリアントの注意点:**  
OBS → mediamtx の直結パスは FFmpeg を通らないため、`-use_wallclock_as_timestamps` が適用されない。ただし mediamtx は受信タイムスタンプを基に PDT を壁時計で付与するため、OBS クロックの微小なドリフトは hls.js が PDT を参照して補正する。
